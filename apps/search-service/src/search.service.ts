...
export class SearchService {
  ...
  async searchUsers(userId: string, params: any) {
    ...
    if (params.relationshipGoal)
      qb.andWhere('p.relationshipGoal = :rg', { rg: params.relationshipGoal });
    const page = params.page || 1;
    const limit = params.limit || 20;
    qb.orderBy('p.completionPercentage', 'DESC')
      .skip((page - 1) * limit)
      .take(limit);
  ...
  async autocomplete(userId: string, query: string) {
    ...
    const profiles = await this.profileRepo
      ...
      .getMany();
    return {
      suggestions: profiles.map((p) => ({ text: p.firstName, type: 'name', userId: p.userId })),
    };
  }
}
