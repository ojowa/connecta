...
export class AdminService {
  ...
  async login(data: any) {
    ...
    await this.sessRepo.save(
      this.sessRepo.create({
        adminId: admin.id,
        tokenHash: tokens.accessToken.substring(0, 20),
  ...
  async getDashboard(period = '7d') {
    ...
    return {
      ...
      revenue: { totalRevenueNgn: totalRevenue?.sum || 0, activeSubscriptions: totalSubs },
      safety: { totalReports },
    };
  }
  ...
  async getUsers(params: any) {
    ...
    if (params.search)
      qb.where('u.fullName ILIKE :s OR u.email ILIKE :s', { s: `%${params.search}%` });
  ...
  async suspendUser(userId: string, data: any) {
    ...
    return {
      userId,
      status: 'suspended',
      suspendedUntil: new Date(Date.now() + (data.durationDays || 30) * 24 * 60 * 60 * 1000),
    };
  }
  ...
  async verify2fa(data: any) {
    ...
    try {
      ...
      await this.auditRepo.save(
        this.auditRepo.create({ adminId: admin.id, action: 'admin.2fa_verify' }),
      );
      return {
        admin: {
          id: admin.id,
  ...
  async getAnalytics(params: any) {
    ...
    const totalRevenue = await this.txnRepo
      .createQueryBuilder('t')
      .select('SUM(t.amount)', 'sum')
      .where('t.status = :status AND t."createdAt" >= :startDate', {
        status: 'completed',
        startDate,
      })
      .getRawOne();
    ...
    for (let i = days; i >= 0; i--) {
      ...
      const dayUsers = await this.userRepo
        ...
        .getCount();
      const dayRevenue = await this.txnRepo
        .createQueryBuilder('t')
        .select('SUM(t.amount)', 'sum')
    ...
    return {
      ...
      revenue: {
        totalInPeriod: parseFloat(totalRevenue?.sum || '0') / 100,
        currency: 'NGN',
        activeSubscriptions,
      },
      safety: {
        ...
        resolvedReports,
        resolutionRate:
          totalReports > 0 ? `${Math.round((resolvedReports / totalReports) * 100)}%` : 'N/A',
      },
      dataPoints,
    };
  }
