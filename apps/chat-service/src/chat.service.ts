...
export class ChatService {
    ...
    const conversations = await Promise.all(
      participations.map(async (p) => {
        ...
        const otherProfile = otherUserId
          ? await this.profileRepo.findOne({ where: { userId: otherUserId } })
          : null;
        const photo =
          otherUserId && otherProfile
            ? await this.photoRepo.findOne({
                where: { profileId: otherProfile.id, isPrimary: true },
              })
            : null;
        ...
        return {
          ...
          participant: otherUser
            ? {
                ...
                avatarUrl: photo?.url,
                isOnline: false,
                lastSeenAt: otherUser.lastActiveAt,
              }
            : null,
          lastMessage: lastMsg
            ? {
  ...
  async getMessages(
    ...
    if (after)
      qb.andWhere('m.createdAt > (SELECT "createdAt" FROM messages WHERE id = :after)', { after });
  ...
  async sendMessage(userId: string, conversationId: string, data: any) {
    ...
    await Promise.all(
      otherParts
        ...
        .map((p) => this.partRepo.update(p.id, { unreadCount: p.unreadCount + 1 })),
    );

    const participantUserIds = otherParts.filter((p) => p.userId !== userId).map((p) => p.userId);
    this.natsClient.emit(CHAT_EVENTS.MESSAGE_SENT, {
      messageId: saved.id,
      conversationId,
      senderId: userId,
  ...
  async deleteMessage(userId: string, messageId: string) {
    ...
    this.natsClient.emit(CHAT_EVENTS.MESSAGE_DELETED, {
      messageId,
      userId,
