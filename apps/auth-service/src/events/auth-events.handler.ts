...
export class AuthEventsHandler {
  ...
  async handleSubscriptionActivated(data: { subscriptionId: string; userId: string; planId: string }) {
    this.logger.log(`Received subscription.activated for ${data.userId} — upgrading role to premium`);
  ...
  async handleSubscriptionCancelled(data: { subscriptionId: string; userId: string; planId: string }) {
    this.logger.log(`Received subscription.cancelled for ${data.userId} — downgrading role to user`);
