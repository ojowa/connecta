...
export class MatchingService {
  ...
  async like(userId: string, targetUserId: string, likeType = 'normal') {
    ...
    if (mutualLike) {
      ...
      if (likeType === 'super') {
        this.eventEmitter.emit(MATCH_EVENTS.SUPER_LIKE_SENT, {
          fromUserId: userId,
    ...
    this.eventEmitter.emit(MATCH_EVENTS.SWIPE_PERFORMED, { userId, targetUserId, type: likeType });
  ...
  async pass(userId: string, targetUserId: string) {
    ...
    this.eventEmitter.emit(MATCH_EVENTS.SWIPE_PERFORMED, { userId, targetUserId, type: 'pass' });
  ...
  async unmatch(userId: string, matchId: string) {
    ...
    this.eventEmitter.emit(MATCH_EVENTS.UNMATCH, { matchId, userId });
