...
export class NotificationsController {
  ...
  getPrefs(@Body('_userId') userId: string) {
    return this.notificationsService.getPreferences(userId);
  }

  @Put('preferences')
  @ApiOperation({ summary: 'Update preferences' })
  updatePrefs(@Body('_userId') userId: string, @Body() body: UpdateNotificationPrefsDto) {
    return this.notificationsService.updatePreferences(userId, body);
  }

  @Put('read')
  @ApiOperation({ summary: 'Mark as read' })
  markRead(@Body('_userId') userId: string, @Body() body: MarkNotificationsReadDto) {
