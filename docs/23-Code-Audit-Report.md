...
## OJChat — Full Implementation Audit
...
| `DB_NAME` | All services | `ojchat_db` |
...
- `apps/users-service/src/users.controller.ts` — no crypto endpoints
- `apps/users-service/src/app.module.ts` — no `PreKeyBundle` entity imported

**Impact:** All `/v1/crypto/prekeys/*`, `/v1/crypto/sessions/*`, `/v1/crypto/backup` routes return 404
**Root cause:** API gateway proxies crypto routes to users-service, but users-service was never given crypto endpoints or the Signal Protocol `PreKeyBundle` entity
**Fix:** Added crypto endpoints to users-service controller, imported `PreKeyBundle` entity in module
...
**File:** `apps/payments-service/src/payments.service.ts`
...
**File:** `apps/notifications-service/src/notifications.service.ts`
...
**File:** `ojchat-mobile/src/store/index.ts:61-65`
...
**File:** `ojchat-mobile/src/screens/settings/SettingsScreen.tsx:58-128`
...
**File:** `ojchat-mobile/src/screens/subscription/SubscriptionScreen.tsx:31-87`
...
**Fix required:** Fetch plans from payments-service API, implement subscribe flow

### 3.4 Video Call Screens — UI Shells

**File:** `ojchat-mobile/src/screens/call/ActiveVideoCallScreen.tsx`
...
**File:** `ojchat-mobile/src/screens/notifications/NotificationsScreen.tsx:55-57`
...
**File:** `ojchat-mobile/src/screens/subscription/WalletScreen.tsx:116-118`
...
**Details:**
- Config service reads `DB_DATABASE` while `.env.example` uses `DB_NAME` (both default to `ojchat_db`)
...
**File:** `apps/notifications-service/src/notifications.service.ts:38`
...
- [x] Add crypto endpoints to users-service
