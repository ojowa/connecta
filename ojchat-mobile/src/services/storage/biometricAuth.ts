...
export class BiometricAuthService {
  ...
  static async getBiometricType(): Promise<'fingerprint' | 'face' | 'iris' | null> {
    ...
    const typeMap: Record<number, 'fingerprint' | 'face' | 'iris'> = { 1: 'fingerprint', 2: 'face', 3: 'iris' };
    return typeMap[types[0]] ?? null;
  }

  static async authenticate(reason: string): Promise<{ success: boolean; error?: string; biometricType: string | null }> {
    const biometricType = await this.getBiometricType();
    if (!biometricType) return { success: false, error: 'Biometric not available', biometricType: null };
    try {
