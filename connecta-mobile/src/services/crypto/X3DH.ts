...
import { PreKeyBundle, X3DHResult, KeyPair } from '../../types/crypto';
...
export class X3DH {
  static async performKeyExchange(
    ...
    const dh1 = await this.computeDH(senderIdentityKeyPair.privateKey, preKeyBundle.signedPreKey);
    const dh2 = await this.computeDH(ephemeralKeyPair.privateKey, preKeyBundle.identityKey);
    const dh3 = await this.computeDH(ephemeralKeyPair.privateKey, preKeyBundle.signedPreKey);
    const dh4 = await this.computeDH(ephemeralKeyPair.privateKey, preKeyBundle.oneTimePreKey);
